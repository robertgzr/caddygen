...
func WriteTempTemplate(id string) (string, error) {
	tpl, err := ioutil.TempFile("", "caddygen-template-")
	if err != nil {
		return "", err
	}
	if _, ok := templates[id]; !ok {
		return "", fmt.Errorf("no such template: %v", id)
	}
	if _, err := tpl.Write([]byte(templates[id])); err != nil {
		return "", err
	}
...
const darkTemplate string = `<!DOCTYPE html>
<html>
	<head>
		<title>{{html .Name}}</title>
		<meta charset="utf-8">
